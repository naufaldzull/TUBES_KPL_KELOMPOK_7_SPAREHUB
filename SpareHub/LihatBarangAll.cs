...
namespace SpareHub
{
    ...
    /// </summary>
    public partial class LihatBarangAll : Form // PascalCase 
    {
        ...
        private Button btnRefresh, btnTutup;
        private Label lblTotal;

        public LihatBarangAll()
        {
            InitializeComponent();
        ...
        private void SetupForm()
        {
            ...
            btnRefresh = CreateButton("Refresh Lokal", new Point(580, 420), Color.LightBlue, (s, e) => LoadData());
            btnTutup = CreateButton("Tutup", new Point(680, 420), Color.LightCoral, (s, e) => Close());
            var btnLihatUlasan = CreateButton("Lihat Ulasan", new Point(480, 420), Color.LightGreen, BtnLihatUlasan_Click);
        }
        ...
        /// <summary>
        /// Event handler tombol Lihat Ulasan. Tampilkan ulasan untuk produk terpilih di MessageBox.
        /// </summary>
        private void BtnLihatUlasan_Click(object sender, EventArgs e)
        {
            if (dgvBarang.SelectedRows.Count != 1)
            {
                MessageBox.Show("Pilih salah satu produk terlebih dahulu.");
                return;
            }

            var row = dgvBarang.SelectedRows[0];
            if (row.Cells["Nama"].Value is not string namaProduk || string.IsNullOrWhiteSpace(namaProduk))
            {
                MessageBox.Show("Nama produk tidak valid.");
                return;
            }

            var reviews = _reviewService.GetSubmittedReviews(namaProduk); // method ini kamu perlu tambahkan
            if (reviews.Count == 0)
            {
                MessageBox.Show("Belum ada ulasan untuk produk ini.");
                return;
            }

            var reviewText = string.Join(Environment.NewLine, reviews.Select(r =>
                $"⭐ {r.Rating} - {r.Reviewer}: {r.Comment}"));

            MessageBox.Show(reviewText, $"Ulasan untuk {namaProduk}");
        }
    }
}